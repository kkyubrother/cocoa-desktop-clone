...
export default function ChatPage() {
  return <main>
    ...
    >
      <h4 style={{fontWeight: "700", display: "flex", justifyContent: "space-between"}}>
        <div>{`채팅`}<span style={{marginLeft: "1rem", fontSize: "1rem", textAlign: "center"}}><BsTriangleFill style={{
          transform: "rotate(180deg)", cursor: "pointer"
        }}/></span></div>
        <div style={{display: "flex", flexDirection: "row", minWidth: "120px", justifyContent: "space-around"}}>
          <div><BsSearch/></div>
          <div><BsWechat/></div>
      ...
      <div>
        <div style={{position: "relative", height: "5rem"}}>
          <BsSearch style={{position: "absolute", top: "1.25rem", left: "1.25rem"}}/><input
          style={{
            borderRadius: "50px",
        ...
        <ChatRow title={"세별"} description={"어둠을 밝히는 별이 되고싶습니다"} icon={IconSamsung} time={"1시간 전"} count={"3"}/>
        <ChatRow title={"버거왕"} description={"와퍼는 맛있죠"} icon={IconBurgerking} time={"12:30"} count={"99"}/>
      </div>
    </div>
  </main>
}
